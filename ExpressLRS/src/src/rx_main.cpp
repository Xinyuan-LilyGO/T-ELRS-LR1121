...
void ICACHE_RAM_ATTR getRFlinkInfo()
{
    ...
    // DBGLN("Rssi1DBM:%d",CRSF::LinkStatistics.uplink_RSSI_1);
    // DBGLN("Rssi2DBM:%d",CRSF::LinkStatistics.uplink_RSSI_2);
#if defined(DEBUG_BF_LINK_STATS)
...
void SetRFLinkRate(uint8_t index, bool bindMode) // Set speed of RF link
{
    ...
    MspReceiver.setMaxPackageIndex(ELRS_MSP_MAX_PACKAGES); // ELRS_MSP_MAX_PACKAGES : 14
...
bool ICACHE_RAM_ATTR HandleFHSS() // 处理频率跳变
{
    uint8_t modresultFHSS = (OtaNonce + 1) % ExpressLRS_currAirRate_Modparams->FHSShopInterval; // modresultFHSS 决定是否进行跳变
...
bool ICACHE_RAM_ATTR HandleSendTelemetryResponse()
{
    ...
    if(AT2401C_EN)
    {
        if (AT2401C_1_TXEN != UNDEF_PIN)
        {
            pinMode(AT2401C_1_TXEN, OUTPUT);
            digitalWrite(AT2401C_1_TXEN, HIGH);
        }
        if (AT2401C_2_TXEN != UNDEF_PIN)
        {
            pinMode(AT2401C_2_TXEN, OUTPUT);
            digitalWrite(AT2401C_2_TXEN, HIGH);
        }
        if (AT2401C_1_RXEN != UNDEF_PIN)
        {
            pinMode(AT2401C_1_RXEN, OUTPUT);
            digitalWrite(AT2401C_1_RXEN, LOW);
        }
        if (AT2401C_2_RXEN != UNDEF_PIN)
        {
            pinMode(AT2401C_2_RXEN, OUTPUT);
            digitalWrite(AT2401C_2_RXEN, LOW);
        }
    }
...
static void ICACHE_RAM_ATTR ProcessRfPacket_RC(OTA_Packet_s const *const otaPktPtr)
{
    ...
    bool telemetryConfirmValue = OtaUnpackChannelData(otaPktPtr, ChannelData, ExpressLRS_currTlmDenom); // OtaUnpackChannelData指向三个不同的解包函数
...
void ICACHE_RAM_ATTR TXdoneISR()
{
    if(AT2401C_EN)
    {
        ...
        {
            pinMode(AT2401C_1_TXEN, OUTPUT);
            digitalWrite(AT2401C_1_TXEN, LOW);
        }
        if (AT2401C_2_TXEN != UNDEF_PIN)
        {
            pinMode(AT2401C_2_TXEN, OUTPUT);
            digitalWrite(AT2401C_2_TXEN, LOW);
        }
        if (AT2401C_1_RXEN != UNDEF_PIN)
        {
            pinMode(AT2401C_1_RXEN, OUTPUT);
            digitalWrite(AT2401C_1_RXEN, HIGH);
        }
        if (AT2401C_2_RXEN != UNDEF_PIN)
        {
            pinMode(AT2401C_2_RXEN, OUTPUT);
            digitalWrite(AT2401C_2_RXEN, HIGH);
        }
    }
    Radio.RXnb();
...
void MspReceiveComplete()
{
    ...
    case MSP_ELRS_SET_RX_WIFI_MODE: // 0x0E
        ...
        // keep sending it, so defer the switch to wifi  defaul 500ms
    ...
    default:
        ...
        case CRSF_FRAMETYPE_MSP_WRITE: // encapsulated MSP payload
            DBGLN("CRSF_FRAMETYPE_MSP_WRITE");
...
static void setupBindingFromConfig()
{
    ...
    OtaUpdateCrcInitFromUid(); // 从UID[4] 和UID[5]计算一个校验值
}

static void setupRadio()
{
    Radio.currFreq = FHSSgetInitialFreq(); // 获取初始频率
    DBGLN("Radio currFreq: %d", Radio.currFreq);
#if defined(RADIO_SX127X)
    // Radio.currSyncWord = UID[3];
#endif
    bool init_success = Radio.Begin(FHSSgetMinimumFreq(), FHSSgetMaximumFreq()); // 初始化无线电
    POWERMGNT::init();
    if (!init_success)
    {
    ...
    DynamicPower_UpdateRx(true); // 初始化电源管理模块

#if defined(Regulatory_Domain_EU_CE_2400)
    LBTEnabled = (config.GetPower() > PWR_10mW);
#endif

    Radio.RXdoneCallback = &RXdoneISR; // 无线电接收回调函数
    Radio.TXdoneCallback = &TXdoneISR; // 无线电发送回调函数

    scanIndex = config.GetRateInitialIdx(); // 获取配置无线通讯速率索引  scanIndex:4
    // 设置无线通信 出现大电流现象
    SetRFLinkRate(scanIndex, false); // 设置无线通讯速率

    // Start slow on the selected rate to give it the best chance
    // to connect before beginning rate cycling
    RFmodeCycleMultiplier = RFmodeCycleMultiplierSlow / 2; // 设置无线通信速率循环 multiplier
}
...
static void EnterBindingMode() // 绑定模式
{
    if (InBindingMode)
    {
...
// to the serial port and they'll block if the buffer fills
        ...
        FHSSrandomiseFHSSsequence(uidMacSeedGet()); // 跳频种子 UID[2]-UID[5]中计算返回macseed
