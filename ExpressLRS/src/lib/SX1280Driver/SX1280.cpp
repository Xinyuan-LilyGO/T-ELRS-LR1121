...
{
    hal.init();
    hal.IsrCallback_1 = &SX1280Driver::IsrCallback_1;//接收回调函数
                          ...
                          uint32_t flrcSyncWord, uint16_t flrcCrcSeed, uint8_t flrc)
{
    ...
    {
        DBG("Config LoRa \n");
        ConfigModParamsLoRa(bw, sf, cr);//bw：带宽  sf：扩频因子  cr：编码率
#if defined(DEBUG_FREQ_CORRECTION)
        SX1280_RadioLoRaPacketLengthsModes_t packetLengthType = SX1280_LORA_PACKET_VARIABLE_LENGTH;
#else
        SX1280_RadioLoRaPacketLengthsModes_t packetLengthType = SX1280_LORA_PACKET_FIXED_LENGTH;//没有包头，固定长度格式
#endif
...
void ICACHE_RAM_ATTR SX1280Driver::SetFrequencyReg(uint32_t regfreq, SX12XX_Radio_Number_t radioNumber)
{
    ...
    currFreq = regfreq;
    // DBGLN("Entering binding mode of currFreq: %d", currFreq);
}
...
{
    // DBGLN("radionumber:%d", radioNumber);
    ...
    if (irqStatus & SX1280_IRQ_TX_DONE)//判断发送中断完成
    {
        RFAMP.TXRXdisable();
        instance->TXnbISR();
        irqClearRadio = SX12XX_Radio_All;
    }
    else if (irqStatus & SX1280_IRQ_RX_DONE)//判断接收中断完成
    {
